...
//             </div>
//             <button className="btn-anim" type="submit">
//               {loading ? <CircularProgress size={21} style={{ color: "white" }} /> : "Send Reset Link"}
//             </button>
//             <div className="mt-3 text-center">
...
export default function Login() {
  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(
        "https://crm-backend-8w9h.onrender.com/customer/login",
        formData,
        {
          headers: { "Content-Type": "application/json" },
        }
  ...
  const handleForgotPasswordSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const res = await axios.post(
        "https://crm-backend-8w9h.onrender.com/customer/forgot-password",
        { email: values.email, organization: "67038bd02edf8e22c9d06a69" },
        {
          headers: { "Content-Type": "application/json" },
        }
      );
  ...
  const handleResetPasswordSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const res = await axios.post(
        `https://crm-backend-8w9h.onrender.com/customer/reset-password/${values.resetToken}`,
        {
          newPassword: values.newPassword,
        }
      );
      ...
      {!isForgotPassword && !isResetPassword ? (
        <form id="loginForm" onSubmit={handleLoginSubmit}>
          <div className="row g-4">
            ...
            <button className="custom-green-btn" type="submit">
              {loading ? (
              ...
              <div className="form-group text-start">
                ...
                <div className="form-control-wrap">
                  <input
                    ...
                    value={values.resetToken}
                    onChange={(e) =>
                      setValues({ ...values, resetToken: e.target.value })
                    }
                  />
                </div>
              </div>
